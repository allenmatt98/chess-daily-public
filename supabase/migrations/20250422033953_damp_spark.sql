/*
  # Add attempt tracking and validation
...
-- Add attempt tracking columns if they don't exist
DO $$
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM information_schema.columns
    WHERE table_name = 'user_progress' AND column_name = 'last_attempt_at'
  ) THEN
    ALTER TABLE user_progress ADD COLUMN last_attempt_at timestamptz;
  END IF;

  IF NOT EXISTS (
    SELECT 1 FROM information_schema.columns
    WHERE table_name = 'user_progress' AND column_name = 'attempts_today'
  ) THEN
    ALTER TABLE user_progress ADD COLUMN attempts_today integer DEFAULT 0;
  END IF;
END $$;
